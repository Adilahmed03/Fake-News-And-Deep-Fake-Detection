...
def init_detectors():
    ...
    try:
        ...
        if os.path.exists(app.config['FAKENEWS_MODEL_PATH']):
            fakenews_detector = FakeNewsDetector(
                model_path=app.config['FAKENEWS_MODEL_PATH'],
                train_data_path=app.config.get('FAKENEWS_TRAIN_DATA')
            )
            print("[OK] Fake News Detector initialized")
        else:
            print(f"[WARNING] Fake news model not found at {app.config['FAKENEWS_MODEL_PATH']}")
            print("  Please copy the model file from Fake_News_Detection/final_model.sav")
